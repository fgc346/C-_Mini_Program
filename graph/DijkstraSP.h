...
using Node_Distance =
    std::pair<int, double*>;  // (v, &distTo[v]) distTo表示s-v的路径代价
struct myGreater {
...
using MyPriorityQueue =
    std::priority_queue<Node_Distance, std::vector<Node_Distance>, myGreater>;
...
class DijkstraSP {
 public:
  ...
  bool HasPathTo(int v) { return distTo_[v] < POSITIVE_INFINITY; }
 ...
 private:
  ...
  // decltype([](DirectedEdge left, DirectedEdge right) -> bool {left.Weight() >
  // right.Weight();})> pq_;
